...
if (!rootUser || !rootPass) {
  console.error(
    "使い方: node test-surreal/test-diagnose.mjs <root-user> <root-pass>",
  );
...
try {
  ...
  try {
    const users = await db.query(
      "SELECT user_id, first_name, last_name, role, string::len(password) AS pw_len FROM user",
    );
  ...
  try {
    const duplicateNames = await db.query(`
      ...
      ORDER BY count DESC
    `);
    console.log("[4] 結果:", JSON.stringify(duplicateNames, null, 2));
  } catch (e) {
  ...
  try {
    const hashResult = await db.query(
      "RETURN crypto::argon2::generate('1234')",
    );
    console.log("[5] ハッシュ例:", hashResult);

    // 既存ユーザーのパスワードと比較テスト
    const compareResult = await db.query(
      "SELECT user_id, last_name, first_name, crypto::argon2::compare(password, '1234') AS pw_match FROM user ORDER BY user_id ASC LIMIT 1",
    );
    console.log(
      "[5] 先頭ユーザーのパスワード比較結果:",
