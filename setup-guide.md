...
### 2-5. 開発用 CLI 接続設定（Claude Code / ターミナルからクエリを実行する場合）

ダッシュボードの「Manage access」→「Root Authentication」の `+` ボタンでrootユーザーを作成する。

| 項目     | 設定値                   |
| -------- | ------------------------ |
| username | 任意（例: `admin`）      |
| password | 十分に強いランダム文字列 |
| role     | Owner                    |

> **注意:** SurrealDB Cloud の Google OAuthログインとは別の認証情報。ダッシュボードのログインパスワードではなく、データベース接続専用のユーザー。

作成後、認証情報を `~/.claude/settings.json` の `env` に追記する（Claude Code の全セッションで自動的に読み込まれる）。

```json
{
  "env": {
    "SURREALDB_ENDPOINT": "wss://your-instance.aws-aps1.surreal.cloud",
    "SURREALDB_USER": "admin",
    "SURREALDB_PASS": "（設定したパスワード）"
  }
}
```

> **注意:** `~/.claude/settings.json` はgit管理外。他の開発者はそれぞれ自分の環境で設定する。

プロジェクトルートに `surql.sh` を作成する（namespace/database はプロジェクトの `.env` から読み込む）。
...
プロジェクトの `.env` に namespace / database を追加する。
...
`Item` / `Want` に、交渉相手を表示するための `requester_user_id?: string` と
`requester_name?: string` を追加する。
...
アイテム一覧と want 一覧を並行取得し、アイテムに希望者の会員IDと名前を付加する。
...
items = (itemResult[0] ?? []).map((item) => ({
  ...item,
  requester_user_id: wantByItemId.get(String(item.id))?.requester_user_id,
...
async function enrichItemWithRequester(item: Item): Promise<Item> {
  ...
  const r = await db.query<[Want[]]>(
    'SELECT requester.user_id AS requester_user_id, requester.last_name + requester.first_name AS requester_name FROM want WHERE item = type::thing("item", $id)',
    { id: idPart },
  );
  return {
    ...item,
