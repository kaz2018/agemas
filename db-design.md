...
DEFINE FIELD last_name  ON user TYPE string;   -- 姓（ひらがな、ログイン・表示用）
DEFINE FIELD first_name ON user TYPE string;   -- 名（ひらがな、ログイン・表示用）
...
DEFINE INDEX user_full_name_unique ON user FIELDS last_name, first_name UNIQUE;
...
-- ログインキーは last_name + first_name + 4桁PIN
...
を付加しておくことで、出品者側に `002（やまだ はなこ）` のような形で表示できる。
