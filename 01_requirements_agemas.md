...
地域内で不要になった物を無料でやり取りするためのコミュニティプラットフォーム。フリマアプリとは異なり、実名制・小規模・信頼ベースの取引を想定。技術ハードルを最小化し、非開発者でも運用・保守可能な設計を最優先。
...
- **初期運用期間**: 3ヶ月テスト運用
- **コスト目標**: 月額¥200-300（Google Cloud Storage のみ）
...
- Google Sheets から認証情報を読み込み
- bcrypt でパスワード検証
- セッション管理は Streamlit の session_state で管理

**FR-1.3: ユーザー停止**
- 運営者が Google Sheet 『master_users』で status を「suspended」に変更
- 停止ユーザーはログイン不可
...
- 自動タイムスタンプ記録
- 画像は Google Cloud Storage に保存、URL を Google Sheet に記録
...
**FR-5.3: ユーザー停止**
- 運営者が Google Sheet 『master_users』の status を変更
...
- **全アクション記録**: 全投稿、返信、状態変更を Google Sheet ログに記録
- **ログ保持期間**: 最低 1年間
- **ログアクセス**: 運営者のみ（Google Sheet を開く権限）
